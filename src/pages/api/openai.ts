...
export default async function handler(
  ...
  try {
    ...
    const responseMessage = completion.data.choices[0]?.message?.content?.trim() || "Ruh-oh, there might be an issue with OpenAI...or Shouryamaan made a mistake which is highly possible. Just text or email Shouryamaan and he'll fix it ASAP!";


    if (!responseMessage) {
      res
        .status(400)
        .json({ error: "Ruh-oh, there might be an issue with OpenAI. Try again....or Shouryamaan made a mistake which is highly possible. Just text or email Shouryamaan and he'll fix it ASAP!" });
    }
